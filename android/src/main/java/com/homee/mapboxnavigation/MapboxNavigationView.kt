...
class MapboxNavigationView(private val context: ThemedReactContext) : FrameLayout(context.baseContext) {
    ...
    /*
     * Camera padding values for route overview and following modes.
     */
    ...
    /**
     * Generates updates for the [MapboxManeuverView].
     */
    private lateinit var maneuverApi: MapboxManeuverApi

    /**
     * Generates updates for the [MapboxTripProgressView].
     */
    private lateinit var tripProgressApi: MapboxTripProgressApi

    /**
     * Stores the state of voice instructions (muted or not).
     */
    ...
    /**
     * Extracts messages for upcoming maneuvers.
     */
    private lateinit var speechApi: MapboxSpeechApi

    /**
     * Plays synthesized audio files or uses Text-To-Speech.
     */
    ...
    /**
     * Handles speech synthesis results.
     */
    ...
    /**
     * Provides location updates to the Maps SDK.
     */
    ...
    /**
     * Renders maneuver arrow mutations.
     */
    ...
    /**
     * Handles route progress updates.
     */
    ...
    private fun configureRouteSimulation() {
        mapboxNavigation?.let { navigation ->
            val routes = navigation.getNavigationRoutes()
            if (routes.isNotEmpty()) {
                val replayRouteMapper = ReplayRouteMapper()
                val replayEvents = replayRouteMapper.mapRouteToReplayEvents(routes.first())
                mapboxReplayer.pushEvents(replayEvents)
                mapboxReplayer.play()
                navigation.setLocationEngine(mapboxReplayer)
            }
        }
    }
    ...
    private fun setRouteAndStartNavigation(routes: List<NavigationRoute>) {
        ...
        if (shouldSimulateRoute) {
            configureRouteSimulation()
        } else {
            mapboxNavigation?.setLocationEngine(navigationLocationProvider)
        }
    ...
    fun setShouldSimulateRoute(shouldSimulateRoute: Boolean) {
        ...
        if (mapboxNavigation != null && shouldSimulateRoute) {
            configureRouteSimulation()
